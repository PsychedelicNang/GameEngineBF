...
bool ParticleSystem::Initialize(ComPtr<ID3D11Device>& _device, WCHAR * _fileName)
{
...
bool ParticleSystem::UpdateFrame(float _deltaTime, ComPtr<ID3D11DeviceContext>& _deviceContext)
{
	bool result;
	...
	result = UpdateBuffers(_deviceContext);
	if (!result)
	{
		return false;
	}
...
void ParticleSystem::Render(ComPtr<ID3D11DeviceContext>& _deviceContext)
{
	// Put the vertex and index buffers on the graphics pipeline to prepare them for drawing.
	RenderBuffers(_deviceContext);
}

ComPtr<ID3D11ShaderResourceView>& ParticleSystem::GetTexture()
{
	return m_texture;
}
...
bool ParticleSystem::LoadTexture(ComPtr<ID3D11Device>& _device, WCHAR * _fileName)
{
...
bool ParticleSystem::InitializeBuffers(ComPtr<ID3D11Device>& _device)
{
	...
	m_vertices = new VertexParticle[m_vertexCount];
	if (!m_vertices)
	{
		return false;
	}
	...
	memset(m_vertices, 0, (sizeof(VertexParticle) * m_vertexCount));
	...
	vertexBufferDesc.ByteWidth = sizeof(VertexParticle) * m_vertexCount;
...
void ParticleSystem::EmitParticles(float _deltaTime)
{
...
void ParticleSystem::UpdateParticles(float _deltaTime)
{
	...
	{
		m_particleList[i].positionY = m_particleList[i].positionY - (m_particleList[i].velocity * _deltaTime * 0.1f);
	}
}
...
bool ParticleSystem::UpdateBuffers(ComPtr<ID3D11DeviceContext>& _deviceContext)
{
	int index, i;
	HRESULT result;
	D3D11_MAPPED_SUBRESOURCE mappedResource;
	VertexParticle* verticesPtr;
	...
	for (i = 0; i<m_currentParticleCount; i++)
	{
		...
		m_vertices[index].uv = XMFLOAT2(0.0f, 1.0f);
		m_vertices[index].color = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;

		// Top left.
		m_vertices[index].position = XMFLOAT3(m_particleList[i].positionX - m_particleSize, m_particleList[i].positionY + m_particleSize, m_particleList[i].positionZ);
		m_vertices[index].uv = XMFLOAT2(0.0f, 0.0f);
		m_vertices[index].color = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;

		// Bottom right.
		m_vertices[index].position = XMFLOAT3(m_particleList[i].positionX + m_particleSize, m_particleList[i].positionY - m_particleSize, m_particleList[i].positionZ);
		m_vertices[index].uv = XMFLOAT2(1.0f, 1.0f);
		m_vertices[index].color = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;
		...
		// Top right.
		m_vertices[index].position = XMFLOAT3(m_particleList[i].positionX + m_particleSize, m_particleList[i].positionY + m_particleSize, m_particleList[i].positionZ);
		m_vertices[index].uv = XMFLOAT2(1.0f, 0.0f);
		m_vertices[index].color = XMFLOAT4(m_particleList[i].red, m_particleList[i].green, m_particleList[i].blue, 1.0f);
		index++;
	}

	// Lock the vertex buffer.
	result = _deviceContext->Map(m_vertexBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return false;
	}
	...
	verticesPtr = (VertexParticle*)mappedResource.pData;

	// Copy the data into the vertex buffer.
	memcpy(verticesPtr, (void*)m_vertices, (sizeof(VertexParticle) * m_vertexCount));

	// Unlock the vertex buffer.
	_deviceContext->Unmap(m_vertexBuffer.Get(), 0);

	return true;
}

void ParticleSystem::RenderBuffers(ComPtr<ID3D11DeviceContext>& _deviceContext)
{
	unsigned int stride;
	unsigned int offset;
	...
	_deviceContext->PSSetShaderResources(0, 1, m_texture.GetAddressOf());
